...
WifiMpi::SendDeviceRegistration(uint32_t rank, uint32_t deviceId, uint32_t nodeId)
{
    ...
    // Send via MPI
    MPI_Request request;
    int result = MPI_Isend(messageBuffer, REG_MESSAGE_SIZE, MPI_BYTE, rank, 
                          WIFI_MPI_DEVICE_REGISTER, MpiInterface::GetCommunicator(), &request);
    
    if (result == MPI_SUCCESS)
    {
        NS_LOG_INFO("Successfully sent device registration via MPI: device " << deviceId 
                    << ", node " << nodeId);
    }
    else
    {
                                 ...
                                 double txPowerDbm)
{
    ...
    MPI_Request request;
    int result = MPI_Isend(messageBuffer, SIMPLE_MESSAGE_SIZE, MPI_BYTE, rank, 
                          WIFI_MPI_TX_REQUEST, MpiInterface::GetCommunicator(), &request);
    
    if (result == MPI_SUCCESS)
    {
        NS_LOG_INFO("Successfully sent transmission request via MPI: device " << deviceId 
                    << ", power " << txPowerDbm << "dBm, packet size " << packetSize << " bytes");
