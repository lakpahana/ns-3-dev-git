...
class WifiMpi
{
  public:
    ...
    /**
     * \brief Send device registration message (static wrapper)
     * \param targetRank The rank to send to
     * \param deviceId The device ID
     * \param sourceRank The source rank
     */
    static void SendDeviceRegistration(uint32_t targetRank, uint32_t deviceId, uint32_t sourceRank);

    /**
     * \brief Send transmission request message (static wrapper)
     * \param targetRank The rank to send to
     * \param deviceId The device ID
     * \param ppdu The PPDU to transmit
     * \param txPower The transmission power
     */
    static void SendTransmissionRequest(uint32_t targetRank, uint32_t deviceId, 
                                      Ptr<const WifiPpdu> ppdu, dBm_u txPower);

    /**
     * \brief Send loss model configuration message (static wrapper)
     * \param targetRank The rank to send to
     * \param lossModel The propagation loss model
     */
    static void SendLossModelConfig(uint32_t targetRank, Ptr<PropagationLossModel> lossModel);

    /**
     * \brief Send delay model configuration message (static wrapper)
     * \param targetRank The rank to send to
     * \param delayModel The propagation delay model
     */
    static void SendDelayModelConfig(uint32_t targetRank, Ptr<PropagationDelayModel> delayModel);
